...
for f in glob.glob("docs/plans/PLAN-*.md"):
    ...
    # 상태 표기(상태: Done / Status: Done / 표의 | Done |)만 Done으로 인정.
    # 본문에 "Done" 단어가 스쳐도(예: "끝나면 Done으로") 오탐하지 않게 폴백 제거.
    if not re.search(r'(?i)(상태|status)\s*[:|]\s*done', s) and not re.search(r'\|\s*Done\s*\|', s):
        continue
    # 골든셋 결과 섹션이 있는가
